...
class AffiliateController extends UserBaseController
{
    ...
    public function users(){
        $user = array_slice($this->user_model->affiliate_users($this->user_id,1),0,50);
        $user = $this->user_model->field('parent_id,user_name,avatar')->where(array('id'=>array('in',$user?:'0'),'avatar'=>array('neq','')))->select();
        $this->assign('user',$user);

        $this->assign('page_title','我的用户');
        $this->display();
    }
