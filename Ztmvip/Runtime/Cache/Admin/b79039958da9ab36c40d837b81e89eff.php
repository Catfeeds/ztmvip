...
<div class="art-nav">
    ...
    <span class="right">
        <form method="get" action="<?php echo U('');?>">
            <input name="category_name" class="text" type="text" value="<?php echo ($_GET['category_name']); ?>" placeholder="分类名称搜索...">
...
<table class="tbl js-list" width="100%">
    ...
    <tbody>
    <?php if($list): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr parent="<?php echo ($vo['parent_id']); ?>">
                <td align="center"><input name="id" type="checkbox" value="<?php echo ($vo['id']); ?>"></td>
                <td><?php echo ($vo['id']); ?></td>
                <td><span></span><?php echo ($vo['category_name']); ?></td>
                <td align="center">
                    <a class="js-rank" href="javascript:;">拖拽排序</a>
                </td>
                <td align="center">
                    <?php if($vo['display'] == 'Y'): ?><a class="js-hidden" href="javascript:;">隐藏</a>
                        <?php else: ?>
                        <a class="js-show" href="javascript:;">显示</a><?php endif; ?>
...
<script>
    var list = $('.js-list');
    list.on('click','.js-show,.js-hidden',function(event){
